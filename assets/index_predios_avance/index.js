window.addEventListener('load', ()=>{
  ...
  const visibleColumns = [
    ...
    { key: "cod_exp",              title: "Código Exp" },
    { key: "cod_pu",               title: "Cód Plano Ubi" },
    { key: "cod_pp",               title: "Cód Plano Per" },
    { key: "cod_pa",               title: "Cód Plano Afec" },
    { key: "fecha_elaboracion",    title: "Fecha de elaboración" },
  ];

  // 👉 'fecha_elaboracion' también oculta para FILTRAR por fecha normalizada
  ...
  let _allRows = [];
  let inlineEditCols = { codigo_mtc: false, nombre_razon_social: false, fecha_elaboracion: false };
  ...
  // CAMBIO: cuando se edite en línea 'fecha_elaboracion' y quede vacío, enviar null a Supabase
  $("#tabla").on("change", "input.cell-input", async function(){
    ...
    // CAMBIO: coerción específica para fecha → null si está vacío
    if (field === "fecha_elaboracion" && (!value || value.trim() === "")) {
      value = null;
    }
    ...
    try{
      ...
      if (field === "fecha_elaboracion") {
        // Refrescar la vista con formato YYYY-MM-DD o vacío
        const td = this.closest("td");
        td.textContent = toDateOnly(value);
      }
    }catch(err){
  ...
  $btnSave.addEventListener("click", async ()=>{
    ...
    detailColumns.forEach(col=>{
      ...
      if(!el) return;
      let v = el.value;
      // CAMBIO: Si es fecha y el input está vacío → null
      if(col.type === "date" && (!v || v.trim() === "")) v = null;

      const prev = _initialValues[col.key] ?? null;
      if(col.type === "date"){
        if(toDateOnly(prev) !== toDateOnly(v ?? "")) payload[col.key] = v;
      } else {
